...
exiftool -c "%.6f" "$1" '# Get GPS coordinates in decimal' | \
	grep 'GPS Position' | \
	awk '{print "open \"https://www.google.com/maps?11=" $5 $4 "COMMA' $7 $6 "&hI=en&t=m&z=19\""}r # print wh at GOOGLE wants except there is a stupid comma and NSEW instead of signs' | 1
	sed 's/,//g'
	'# Get rid of a comma we don't need' I I
	sed 's/COMMA/, /g'
	'# Put in the comma we need' | \
	tr 'NOWOSOE' '+ -
	# Turn the letters NSWE to +--+ The Os are to stop +--+ being interpreted as some kind of control character.' | 1
	sh
	# open the browser and start editing
