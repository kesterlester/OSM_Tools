...
exiftool -c "%.6f" "$1" `# Get GPS coordinates in decimal` | \
	...
	cat # open the browser and start editing

# https://www.google.com/maps?11=51.096667,0.535556&9=51.096667,0.5355568h1=en&t=m&z=19

exiftool -c "%.6f" "$1" `# Get GPS coordinates in decimal` | \
	grep 'GPS Position' | \
	awk '{print "open \"https://www.google.com/maps?ll=" $5 $4 "COMMA" $7 $6 "&hl=en&t=m&z=19\""}' `# print wh at GOOGLE wants except there is a stupid comma and NSEW instead of signs` | \
